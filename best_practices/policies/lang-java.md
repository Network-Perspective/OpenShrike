---
id: lang-java
title: Java Language Policy
kind: language
includes: [shared-foundation]
check-count: 71
---

# Policy: Java Language Policy

Merged Java policy. It inlines the baseline `shared-foundation` checks and adds Java-specific checks, including the language checks that previously lived in separate extended outputs. Enable for repositories with substantial Java code. This policy already contains the shared foundation checks; add other `lang-*` policies only when the repo has another substantial implementation language.

## Language-Specific Checks

### Any app type
- [data-java-011](../checks/java/data-java-011-transactions-declare-rollback-for-checked-failures.md) — Java transactions declare rollback for checked failures.
- [rel-java-004](../checks/java/rel-java-004-startup-critical-configuration-properties-are-validated.md) — Startup-critical configuration properties are validated.
- [java-rel-002](../checks/java/java-rel-002-thread-interrupts-are-not-swallowed.md) — Thread interrupts are not swallowed.
- [java-rel-001](../extended/java/java-rel-001-http-clients-define-explicit-time-budgets.md) — Outbound HTTP clients define explicit time budgets.
- [java-sec-003](../extended/java/java-sec-003-parameterized-sql.md) — SQL remains parameterized.
- [java-sec-002](../extended/java/java-sec-002-process-execution-validates-executable-and-arguments.md) — Process execution validates executable and arguments.
- [api-java-002](../extended/java/api-java-002-enums-do-not-use-ordinal-position.md) — Java enums do not use ordinal position for persisted or external semantics.
- [java-arch-001](../extended/java/java-arch-001-constructor-injection-over-field-injection.md) — Runtime dependencies are explicit through constructors.
