...
import type {Finding} from '../scan-data.js';
      ...
      <body>
        <main>
          ...
          <header class="hero">
            ...
            <p class="hero-copy">${escapeHtml(finding.detailSummary)}</p>
          </header>
...
function renderSnippet(snippet: Finding['evidence'][number]['codeSnippet']): string {
  if (!snippet) {
    return '';
  }
...
): NonNullable<Finding['evidence'][number]['codeSnippet']> | undefined {
...
function renderEvidenceCopy(evidence: Finding['evidence'][number], pathLabel: string): string {
