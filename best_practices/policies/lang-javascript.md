---
id: lang-javascript
title: JavaScript Language Policy
kind: language
includes: [shared-foundation]
check-count: 71
---

# Policy: JavaScript Language Policy

Merged JavaScript policy. It inlines the baseline `shared-foundation` checks and adds JavaScript-specific checks, including the language checks that previously lived in separate extended outputs. Enable for repositories with substantial JavaScript code. This policy already contains the shared foundation checks; add other `lang-*` policies only when the repo has another substantial implementation language.

## Language-Specific Checks

### Any app type
- [ops-javascript-001](../checks/javascript/ops-javascript-001-variable-size-async-fan-out-is-bounded.md) — Variable-size async fan-out is bounded.
- [javascript-rel-001](../extended/javascript/javascript-rel-001-http-requests-have-explicit-cancellation-or-timeout.md) — Outbound HTTP requests have explicit cancellation or timeout.
- [javascript-sec-002](../extended/javascript/javascript-sec-002-no-dynamic-code-eval-on-untrusted-input.md) — Dynamic code evaluation is not fed by untrusted input.

### HTTP Service
- [rel-javascript-001](../checks/javascript/rel-javascript-001-request-path-event-loop-backpressure.md) — Request-path code does not block the event loop and preserves backpressure.

### HTTP Service, Batch Job, CLI
- [rel-javascript-002](../extended/javascript/rel-javascript-002-crash-stop-after-uncaught-exceptions-and-handle-eventemitter-errors.md) — Processes crash-stop after uncaught exceptions and EventEmitter error paths are handled.

### HTTP Service, Batch Job, CLI, Library
- [javascript-arch-001](../extended/javascript/javascript-arch-001-module-load-paths-avoid-runtime-side-effects.md) — Module load paths avoid runtime side effects.

### HTTP Service, Batch Job, CLI, Library, Other
- [javascript-sec-001](../extended/javascript/javascript-sec-001-child-process-execution-avoids-shell-injection.md) — Child process execution avoids shell injection.

### Web Frontend
- [data-javascript-011](../extended/javascript/data-javascript-011-indexeddb-writes-stay-inside-active-transaction-callback-stack.md) — IndexedDB writes stay inside the active transaction callback stack.
