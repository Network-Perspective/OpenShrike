...
describe('OpenShrike scan controller', () => {
  ...
  it('refreshes duration while a scan is running without waiting for progress events', async () => {
    ...
    try {
      const runPromise = controller.runScan(workspace);
      await waitForState(model, state => state.canCancel);
...
function waitForState(
  model: {getState(): ReturnType<(typeof createEmptyScanState)>; subscribe(listener: () => void): () => void},
  predicate: (state: ReturnType<(typeof createEmptyScanState)>) => boolean
): Promise<void> {
  if (predicate(model.getState())) {
