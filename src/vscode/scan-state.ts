...
function buildFinding(
  ...
  return {
    ...
    confidence: input.result
      ? input.result.confidence.toLowerCase() as Exclude<Finding['confidence'], null>
      : null,
    summary: isTransient
      ? describeTransientSummary(input.status as Extract<FindingStatus, 'pending' | 'running' | 'fixing'>)
...
function describeTransientSummary(status: Extract<FindingStatus, 'pending' | 'running' | 'fixing'>): string {
  switch (status) {
    case 'pending':
...
function describeTransientRationale(status: FindingStatus): string {
  switch (status) {
    case 'pending':
      return 'Check is pending execution.';
    case 'running':
...
function buildEvidenceItem(raw: string): EvidenceItem {
