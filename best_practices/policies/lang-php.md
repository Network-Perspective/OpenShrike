---
id: lang-php
title: PHP Language Policy
kind: language
includes: [shared-foundation]
check-count: 66
---

# Policy: PHP Language Policy

Merged PHP policy. It inlines the baseline `shared-foundation` checks and adds PHP-specific checks, including the language checks that previously lived in separate extended outputs. Enable for repositories with substantial PHP code. This policy already contains the shared foundation checks; add other `lang-*` policies only when the repo has another substantial implementation language.

## Language-Specific Checks

### Any app type
- [perf-php-001](../extended/php/perf-php-001-strip-loaded-eloquent-relations-before-queue-serialization.md) — Strip loaded Eloquent relations before queue serialization.
- [rel-php-004](../extended/php/rel-php-004-logger-context-handling-safe-for-arbitrary-values.md) — Logger context handling is safe for arbitrary values.

### HTTP Service
- [data-php-007](../checks/php/data-php-007-laravel-request-input-validated-before-persistence.md) — Laravel request input is validated before persistence or business logic uses it.
