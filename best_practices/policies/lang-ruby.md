---
id: lang-ruby
title: Ruby Language Policy
kind: language
includes: [shared-foundation]
check-count: 66
---

# Policy: Ruby Language Policy

Merged Ruby policy. It inlines the baseline `shared-foundation` checks and adds Ruby-specific checks, including the language checks that previously lived in separate extended outputs. Enable for repositories with substantial Ruby code. This policy already contains the shared foundation checks; add other `lang-*` policies only when the repo has another substantial implementation language.

## Language-Specific Checks

### Any app type
- [data-ruby-028](../checks/ruby/data-ruby-028-queued-jobs-pass-stable-identifiers.md) — Queued jobs pass stable identifiers instead of mutable record state.
- [data-ruby-012](../checks/ruby/data-ruby-012-rails-business-invariants-enforced-in-database-and-model.md) — Rails business invariants are enforced in the database as well as the model.
- [data-ruby-011](../extended/ruby/data-ruby-011-atomic-file-replacement.md) — Ruby file-backed durable state uses atomic replacement.
