...
export async function writeShrikeInitFiles(options: InitWriteOptions): Promise<InitWriteResult> {
  ...
  if (policyIds.length === 0) {
    throw new Error('At least one policy must be selected.');
  }
...
export function buildShrikeProjectConfig(options: {
  ...
  return {
    ...
    init: {
      ...
      detectedFrom: options.detectedFrom,
      opencodeSetup: options.opencodeSetup,
      seedPolicyId: policyIds[0],
      seedPolicyIds: policyIds
    },
    runtime: {
...
export function buildInitReadme(options?: {
  ...
  return [
    ...
    `- \`${projectConfigLabel}\` stores repo-local Shrike defaults such as the selected policies and scan settings.`,
...
async function seedProjectChecksDirectory(options: {
  ...
  for (const policyId of normalizePolicyIds(options.policyIds)) {
    ...
    for (const checkId of policy.checkIds) {
      const normalizedCheckId = checkId.toLowerCase();
      if (seenCheckIds.has(normalizedCheckId)) {
        continue;
      }
  ...
  const nextInit = patch === 'full'
    ...
    : patch === 'policy'
      ? {
          ...
          seedPolicyIds: projectConfig.init.seedPolicyIds
        }
      : baseInit;
