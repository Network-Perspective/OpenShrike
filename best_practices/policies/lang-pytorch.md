---
id: lang-pytorch
title: PyTorch Language Policy
kind: language
includes: [shared-foundation]
check-count: 71
---

# Policy: PyTorch Language Policy

Merged PyTorch policy. It inlines the baseline `shared-foundation` checks and
adds PyTorch-specific checks, including the language checks that previously
lived in separate extended outputs. Enable for repositories with substantial
PyTorch code. This policy already contains the shared foundation checks; add
other `lang-*` policies only when the repo has another substantial
...
- [pytorch-rel-003](../checks/pytorch/pytorch-rel-003-device-placement-is-explicit-and-consistent.md) — Device placement is explicit and consistent.
- [test-pytorch-004](../checks/pytorch/test-pytorch-004-torch-empty-fully-initialized-before-use.md) — PyTorch tensors allocated with torch.empty are fully initialized before use.
- [pytorch-ops-001](../checks/pytorch/pytorch-ops-001-checkpoints-store-portable-state.md) — Checkpoints store portable state rather than whole model objects.
- [pytorch-rel-002](../extended/pytorch/pytorch-rel-002-inference-disables-gradient-tracking.md) — Evaluation and inference disable gradient tracking.
- [test-pytorch-001](../extended/pytorch/test-pytorch-001-reproducible-paths-seed-every-rng.md) — Reproducible PyTorch paths seed every RNG they use.
- [test-pytorch-003](../extended/pytorch/test-pytorch-003-reproducible-multi-worker-dataloaders-seed-worker-processes.md) — Reproducible multi-worker DataLoaders seed worker processes.
- [test-pytorch-002](../extended/pytorch/test-pytorch-002-deterministic-cuda-paths-disable-cudnn-benchmarking.md) — Deterministic CUDA paths disable cuDNN benchmarking.
